  ...
  Future<void> _register() async {
    ...
    try {
      ...
      if (_role == 'patient') {
        ...
        if (_nameCtrl.text.trim().isEmpty) throw Exception('Please enter your name.');
        if (_ageCtrl.text.trim().isEmpty) throw Exception('Please enter your age.');
          ...
          body: NenaviPage(
            ...
            child: Column(
              ...
              children: [
                // ── Header ───────────────────────────────────────────────
                SpeakableText(
                  text: Localization.get('welcome_nenavi', lang),
                  language: lang,
                  style: NenaviTheme.heading(color: NenaviTheme.accent),
                ),
                const SizedBox(height: 6),
                SpeakableText(
                  text: Localization.get('fill_details', lang),
                  language: lang,
                ...
                // ── Role ──────────────────────────────────────────────────
                SpeakableText(
                  text: Localization.get('role_question', lang),
                  language: lang,
                  style: NenaviTheme.label(),
                ),
                const SizedBox(height: 8),
                ...
                if (_role == 'patient') ...[
                  SpeakableText(
                    text: Localization.get('name', lang),
                    language: lang,
                    style: NenaviTheme.label(),
                  ),
                  const SizedBox(height: 8),
                  ...
                  SpeakableText(
                    text: Localization.get('age', lang),
                    language: lang,
                    style: NenaviTheme.label(),
                  ),
                  const SizedBox(height: 8),
                  ...
                  SpeakableText(
                    text: Localization.get('caregiver_email', lang),
                    language: lang,
                    style: NenaviTheme.label(),
                  ),
                  const SizedBox(height: 8),
                ...
                Center(
                  child: TextButton(
                    ...
                    child: SpeakableText(
                      text: Localization.get('already_have_account', lang),
                      language: lang,
...
class _RoleSelector extends StatelessWidget {
  ...
  const _RoleSelector({required this.value, required this.lang, required this.onChanged});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? NenaviTheme.primary : Colors.white.withValues(alpha: 0.7),
          borderRadius: BorderRadius.circular(14),
          border: Border.all(
            color: selected ? NenaviTheme.primary : NenaviTheme.secondary.withValues(alpha: 0.4),
            width: 2,
          ),
        ),
