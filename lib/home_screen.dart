...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _startAssessment() async {
    ...
    try {
      final attResult = await Navigator.push(context, MaterialPageRoute(
        builder: (ctx) => AttentionTaskScreen(
          language: _language,
          onComplete: (s, seed, num) => Navigator.pop(ctx, [s, seed, num]),
        ),
      ));
      ...
      final encodedWords = await Navigator.push(context, MaterialPageRoute(
        builder: (ctx) => WordRecallEncodingScreen(
          language: _language, onComplete: (w) => Navigator.pop(ctx, w),
        ),
      )) as List<String>?;
      if (!mounted || encodedWords == null) return;

      final incidentalScore = await Navigator.push(context, MaterialPageRoute(
        builder: (ctx) => IncidentalMemoryScreen(
          language: _language, seedPhrase: seedPhrase,
          onComplete: (s) => Navigator.pop(ctx, s),
        ),
      )) as int?;
      if (!mounted || incidentalScore == null) return;

      final orientationScore = await Navigator.push(context, MaterialPageRoute(
        builder: (ctx) => OrientationScreen(
          language: _language, onComplete: (s) => Navigator.pop(ctx, s),
        ),
      )) as int?;
      if (!mounted || orientationScore == null) return;

      final calcScore = await Navigator.push(context, MaterialPageRoute(
      ...
      final delayedScore = await Navigator.push(context, MaterialPageRoute(
        builder: (ctx) => DelayedWordRecallScreen(
          language: _language, originalWords: encodedWords,
          onComplete: (s) => Navigator.pop(ctx, s),
        ),
      )) as int?;
      ...
      final total = attScore + incidentalScore + orientationScore +
                    calcScore + delayedScore;
      final composite = (total * 100) ~/ maxTotal;
      final today = DateTime.now().toIso8601String().split('T')[0];
      final domainMap = {
        'attention': attScore, 'incidental_memory': incidentalScore,
        'orientation': orientationScore, 'calculation': calcScore,
      ...
      await DatabaseHelper().insertScore({
        ...
        'difficulty': 'Basic', 'patient_uid': user?.uid,
      });
      ...
      if (user != null) {
        try {
          await FirebaseFirestore.instance
              ...
              .set({
            'patientUid': user.uid, 'date': today,
            'compositeScore': composite, 'domainScores': domainMap,
            'difficulty': 'Basic', 'timestamp': FieldValue.serverTimestamp(),
          });
        } catch (e) {
        ...
        child: Column(
          ...
          children: [
            ...
            _BigActionCard(
              ...
              subtitle: 'Takes about 5–10 minutes.\nDo it at the same time each day.',
              buttonLabel: _isAssessing ? 'In progress…' : 'Start Today\'s Test',
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: scoreTextColor(score).withOpacity(0.25), width: 1.5),
      ),
      child: Row(children: [
        CircleAvatar(
          radius: 34,
          ...
          child: Text('$score',
              style: NenaviTheme.heading(color: scoreTextColor(score))
                  .copyWith(fontSize: 26)),
        ),
        const SizedBox(width: 16),
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Last Score', style: NenaviTheme.label(color: scoreTextColor(score))),
          Text('$score / 100',
              style: NenaviTheme.subheading(color: scoreTextColor(score))),
          Text(date, style: NenaviTheme.small(color: scoreTextColor(score))),
        ]),
      ]),
    );
  }
...
class _BigActionCard extends StatelessWidget {
  ...
  const _BigActionCard({
    ...
    required this.buttonLabel, this.onTap, this.loading = false,
  });
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          ...
          Expanded(child: Column(
            ...
            children: [
              Text(title, style: NenaviTheme.subheading(color: NenaviTheme.accent)),
